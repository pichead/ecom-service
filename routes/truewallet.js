...
router.post("/redeem", client_auth, async function (req, res, next) {
  try {
    let hash = req.body.hash;
    let phone2 = "0988301026";
    ...
    if(foundClient){
      ...
      const response = await axios.post(`https://gift.truemoney.com/campaign/vouchers/${hash}/redeem`, postData, {
        headers: headers,
        httpsAgent: new (require('https')).Agent({ rejectUnauthorized: false }), // ถ้าคุณต้องการละเว้นการตรวจสอบ SSL certificate
      });
  
      if(response.status === 200 &&response.data.status.message === 'success'){
  
        let redeemAmount = parseFloat(response.data.data.voucher.redeemed_amount_baht)
  
        let create_Transaction = await prisma.credit_transaction.create({
          data: {
            userId:foundClient.id,
            type:"deposit",
            credit:redeemAmount,
            detail:"deposit by truemony gift",
            remark:"operation by system",
          }
        })

        let updateCreditClient = await prisma.user.update({
          where:{
            id:foundClient.id,
          },
          data:{
            credit: foundClient.credit + redeemAmount
          }
        })

        if(create_Transaction && updateCreditClient){
          await prisma.$disconnect();
          res.status(200).send({
            statusCode: 200,
            message: "Success redeem truewallet gift",
            messageTH: "เติมเงินด้วย truewallet gift สำเร็จ",
          ...
          res.status(400).send({
            statusCode: 400,
            message: "Error update transaction contact admin",
            messageTH: "update transaction ไม่สำเร็จ กรุณาติดต่อ admin"
          });
          return;
        }
      }
        ...
        res.status(400).send({
          statusCode: 400,
          message: "Error redeem truewallet gift",
          messageTH: "เติมเงินด้วย truewallet gift ไม่สำเร็จ"
        });
        return;
      }
    }
